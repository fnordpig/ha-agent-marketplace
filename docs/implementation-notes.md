...
The MCP-backed plugins were bumped to `0.1.2` after moving root `.mcp.json` templates so Codex does not reuse stale plugin cache entries that still auto-started placeholder MCP servers.
