...
| `ha-context-official` | Official Home Assistant MCP Server | `plugins/ha-context-official/docs/templates/mcp.json` plus setup script | HTTP | `home-assistant-official` at `http(s)://<host>/api/mcp` | Canonical official endpoint. Use `ha-official-mcp-setup` to write user-level Codex MCP config from inside Codex. |
